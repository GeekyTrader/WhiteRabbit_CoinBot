...
class Class_whale:
    def __init__(self, arg, auth):
        ...
        if self.coin.upper() == "USDT":
            ...
            self.key_binance = self.coin.upper() + "BTC"
    ...
    async def function_fetch(self):
        ...
        list_urls = ["https://bittrex.com/api/v1.1/public/getorderbook?market=" + self.key_rex + "&type=both",
                     ...
                     "https://api.binance.com/api/v1/depth?symbol=" +self.key_binance + "&limit=500",
                     "https://api.kraken.com/0/public/Depth?pair=" + self.key_kraken + "&count=250"]
        list_name = ["Bittrex", "Bitfinex", "Poloniex", "Cryptopia", "Binance", "Kraken"]
            ...
            if exchange[1] == "Binance":
                try:
                    for i in exchange[0]["bids"]:
                        if float(i[1]) * float(i[0]) >= seuil:
                            book_sorted.append(
                                ["Binance", "Sell", float(i[1]) * float(i[0]), float(i[0])])
                    for i in exchange[0]["asks"]:
                        if float(i[1]) * float(i[0]) >= seuil:
                            book_sorted.append(
                                ["Binance", "Buy", float(i[1]) * float(i[0]), float(i[0])])
                except Exception as e:
                    print(e)
